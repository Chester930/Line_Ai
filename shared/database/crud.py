...
class UserCRUD(BaseCRUD):
    ...
    def get_by_line_id(self, line_user_id: str) -> Optional[User]:
        """根據 LINE 用戶 ID 獲取用戶"""
        return self.session.query(User).filter_by(line_user_id=line_user_id).first()
    
    def create_user(self, line_user_id: str, nickname: Optional[str] = None) -> User:
        """創建新用戶"""
        user = User(
...
class ConversationCRUD(BaseCRUD):
    ...
    def create_conversation(self, user_id: int, model: str) -> Conversation:
        """創建新對話"""
